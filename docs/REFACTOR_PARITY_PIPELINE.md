...
## Completed Slice: Narrative Agent Removal

Narrative Craft, Prose Guardian, Narrative Director, and Secret Plot Driver no longer appear in the agent catalog or participate in generation. Stale activation IDs and cached injections are discarded at read boundaries, while historical configuration, run, and memory rows remain stored and inert for compatibility.
