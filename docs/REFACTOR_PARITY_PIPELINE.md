...
6. UI/settings/integration parity: remaining stale controls, loading states, knowledge sources, GIFs, image generation, sprites, and app-level browser passes.
...
| Agents                      | Agent enablement, prompt injection, tool calls, custom tools, agent memory/runs, UI state                | In progress | Custom tools support static results and webhooks in the refactor runtime; legacy script tools are preserved as unsupported rows with a convert-to-static-or-webhook migration path and must not execute. Narrative Craft replaces three overlapping writing agents, keeps legacy activation and memory compatibility, persists one bounded state object, and runs on assistant-message cadence; saved agent runs include config IDs and display names for UI parity; chat-scoped built-in fallback agents resolve without DB rows; empty agent responses surface as failures; preset `agent_data` markers receive runtime output; parallel agent results are not double-counted. Focused tests pass. |
...
| Deki-senpai                 | Persistent history, connection/tool requirements, compaction, loading state, animation                   | In progress | History no longer flashes welcome before load; persona and connection preferences persist; compaction/history tests pass. Need app/browser pass.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| Integrations                | TTS, Spotify, knowledge sources, GIFs, image generation, sprites                                         | In progress | Spotify mini player and TTS audio format restored; haptic/Lovense integration support was removed as an intentional divergence; frontend and Rust checks pass.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| Integrations: haptic/Lovense device control  | Legacy haptic routes and agents could connect through Intiface/Buttplug and control Lovense or compatible devices when enabled.                             | De-Koi does not expose haptic/Lovense device control. The built-in agent, chat command parser, UI controls, shared API wrappers, Tauri commands, HTTP command names, and Buttplug dependencies are removed.                                                                                                                                                                                        | Device actuation through chat generation is outside De-Koi's supported integration scope, so legacy haptic commands are treated as retired rather than remote-only or embedded-only behavior.                   | Historical Pasta sources: https://github.com/Pasta-Devs/Marinara-Engine/issues/1921 and https://github.com/Pasta-Devs/Marinara-Engine/issues/2050 |
...
- Audit remaining agent memory-like outputs against the Pasta staging branch.
...
## Completed Slice: Deki-senpai Loading And Generation Parameters

This slice tightened the next broken user-facing paths:

- Deki-senpai waits for stored history before rendering the welcome message, shows an app-styled restoring state, and keeps her sprite animated while loading.
...
## Completed Slice: Narrative Craft And Legacy Writing-Agent Compatibility

Narrative Craft replaces Prose Guardian, Narrative Director, and Secret Plot Driver with one default-off, cadence-gated background story critic:

- The retired IDs `prose-guardian`, `director`, and `secret-plot-driver` remap to `narrative-craft` during activation and are deduplicated, so any combination resolves to one runtime agent.
- Existing agent configuration rows, Secret Plot memory rows, and historical agent-run rows are preserved rather than rewritten or deleted.
- When no current Narrative Craft `state` exists, legacy `overarchingArc`, active `sceneDirections`, and `pacing` are converted lazily into the bounded current state. Fulfilled legacy directions do not become new obligations.
- New memory uses one `agent-memory` row with `key = "state"`. It tracks pacing, a bounded set of main/subplot threads, open questions, withheld information, unresolved consequences, recent shape choices, last guidance, one consume-once `pendingGuidance` directive, and the last analysis reason.
- Historical `director_event` and `secret_plot` result types remain parseable for old run history, but new Narrative Craft executions produce `context_injection` results.
- Automatic analysis runs only after the assistant reply is saved and visible completion is emitted. A cheap recurrence filter must find the same high-confidence prose habit or non-trivial sentence opening across different assistant turns before the background LLM runs.
- A new foreground generation for the same chat aborts any in-flight Narrative Craft analysis so the critic cannot compete with the writer for provider capacity.
- Validated guidance is cached for a later generation and claimed once without blocking the current reply. `lastGuidance` remains available for inspection after `pendingGuidance` is cleared.
- New chats do not activate Narrative Craft by default. Existing explicit Narrative Craft or retired-agent activation remains compatible.
- Regeneration does not replay current or retired narrative guidance. Narrative Craft re-analyzes the original completed response when explicitly retried.
- `showNarrativeCraftPanel` owns the current panel setting. If it is absent, `showSecretPlotPanel: true` remains a read-only visibility fallback; the next settings change writes the current field without needing to delete unrelated metadata.
- Saved agent run rows continue to carry both `agentConfigId` and `agentName`, preserving the joined run/config shape expected by the roleplay agents UI.
