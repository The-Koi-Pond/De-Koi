...
Enforce the contract at the Roleplay scene-plan owner with a sentence-aware character ceiling and script-shape validation. Planner output with unrequested quoted dialogue or more than three sentences is replaced by a neutral beat directive; exact dialogue remains allowed only when those words appear in the user's request. The local fallback labels the bounded user premise instead of inventing narration. This prevents both long and short scripted planner responses from becoming a second high-authority prose sample. The downstream Roleplay writer, preset, character examples, and scene-opening routing remain unchanged.
