...
**Goal:** Make De-Koi's dense UI easier to read and operate by introducing semantic readability and target-size foundations plus keyboard-visible actions.

**Architecture:** Keep the change in the UI feature lane. Global CSS defines reusable presentation contracts and selected feature components opt into those contracts. Preserve the responsive settings navigation already present on `origin/main`.
...
- Existing settings navigation, IDs, and persisted store values must not change.
...
- [ ] Migrate persistent prose, labels, counters, and actions in the two settings owners to `.de-koi-caption` or `.de-koi-label`; leave purely graphical schedule density markers exempt only when accompanied by readable text.
- [ ] Raise the existing SettingsPanel navigation descriptions to the readable caption contract without changing navigation structure or IDs.
