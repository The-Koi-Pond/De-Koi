...
mod tests {
    ...
    async fn export_blocking_work_leaves_tokio_available() {
        ...
        tokio::time::timeout(PROBE_TIMEOUT, entered_rx)
            .await
            .expect("export operation should enter its blocking body before the deadline")
            .expect("export operation should notify the Tokio test");
        ...
        assert_eq!(
            tokio::time::timeout(PROBE_TIMEOUT, work)
                .await
                .expect("export worker should complete before the deadline")
