...
describe("setup chat launch orchestration", () => {
  ...
  it("continues with defaults only when explicitly requested after a starred preset failure", async () => {
    let recovery = null as import("../../../../engine/onboarding").SetupJourneyRecovery | null;
    const createChat = vi.fn().mockResolvedValue({ id: "chat-1" });
    const applyStarredPreset = vi.fn().mockRejectedValue(new Error("preset unavailable"));
    const complete = vi.fn();
    const launch = createSetupChatLaunchOrchestrator({
      createChat,
      applyStarredPreset,
    ...
    await expect(launch.launch(request)).rejects.toThrow("preset unavailable");
    await expect(launch.launch(request, { skipStarredPreset: true })).resolves.toEqual({ id: "chat-1" });
