...
describe("chat surface character summary query", () => {
  it("loads active chat characters without heavyweight card payloads", async () => {
    vi.mocked(storageApi.list).mockResolvedValue([]);

    useChatSurfaceCharacterSummariesByIds(["character-1"]);
    const query = vi.mocked(useQuery).mock.calls[0]?.[0] as unknown as { queryFn: () => Promise<unknown> };
    await query.queryFn();
    ...
    const options = vi.mocked(storageApi.list).mock.calls[0]?.[1] as
      ...
      | undefined;
    expect(options?.fieldSelections?.data).not.toEqual(
      expect.arrayContaining(["character_book", "alternate_greetings", "first_mes"]),
    );
  });
});
