...
async function createIfMissing<T = Record<string, unknown>>(
  ...
  return storageApi.create<T>(entity, { id, ...value });
}
...
export async function ensureNoModelGameShowcase(): Promise<{ chatId: string }> {
  await createIfMissing("personas", PERSONA_ID, {
    name: "Ari Vale",
  ...
  await createIfMissing("lorebooks", LOREBOOK_ID, {
    name: "Glasswake Harbor",
  ...
  await createIfMissing("lorebook-entries", "showcase-no-model-game-v1-lore-entry-bells", {
  ...
  await createIfMissing("lorebook-entries", "showcase-no-model-game-v1-lore-entry-token", {
    lorebookId: LOREBOOK_ID,
    name: "Tide-token",
    content:
  ...
  await createIfMissing<Chat>("chats", NO_MODEL_GAME_SHOWCASE_CHAT_ID, {
      ...
      gameWidgetState: [
        {
          ...
          label: "Party",
          position: "hud_left",
          config: { stats: [{ name: "Lead", value: "Silent bell" }, { name: "Token", value: "Warm" }] },
        },
        {
          id: "showcase-world",
          type: "stat_block",
          label: "World",
          position: "hud_right",
          config: { stats: [{ name: "Location", value: "Moonlit Dock" }, { name: "Mood", value: "Watchful" }] },
        },
      ],
