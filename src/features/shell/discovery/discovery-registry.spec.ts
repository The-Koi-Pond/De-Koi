...
describe("discovery showcase registry", () => {
  ...
  it("routes Roleplay workflow profiles to the active chat chooser", () => {
    ...
    expect(summary).toContain("Simple Roleplay");
    expect(summary).toContain("Long-Running Story");
    expect(summary).toContain("Cinematic Roleplay");
    expect(summary).toContain("Local Helpers");
    expect(summary.toLowerCase()).toContain("choose");
    expect(entry?.actions).toContainEqual({
  ...
  it("describes both Memory Recall scopes and how automatic capture and embeddings behave", () => {
    ...
    expect(summary).toContain("undo the latest");
    expect(summary).not.toContain("apply cleanup");
    expect(summary).not.toContain("removal review");
