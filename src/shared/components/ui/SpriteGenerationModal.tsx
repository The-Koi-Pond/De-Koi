...
export function SpriteGenerationModal({
  ...
  const requestGeneratedSheet = useCallback(
    async (expressions: string[], grid: SpriteGrid, matchedFullBodyMode: boolean): Promise<GenerateSheetResult> => {
      ...
      const payload = {
        ...
        // Keep provider output raw so preview cleanup can preserve originals.
        noBackground: false,
      };
  ...
  const cleanupSpriteCells = useCallback(
    async (targetCells: SlicedCell[]) => {
      if (targetCells.length === 0) return targetCells;
  ...
  const generateMatchedFullBodyBatch = useCallback(
    async (batchExpressions: string[], batchIndex: number, totalBatches: number) => {
      ...
      for (let attempt = 0; attempt < 2; attempt += 1) {
        ...
        try {
          ...
          const generated = createGeneratedSpritesFromResult(result, grid, `Batch ${batchIndex + 1}`);
          return noBackground ? { ...generated, cells: await cleanupSpriteCells(generated.cells) } : generated;
        } catch (err) {
    ...
    [cleanupSpriteCells, noBackground, requestGeneratedSheet],
  );
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      const generatedCells = noBackground ? await cleanupSpriteCells(generated.cells) : generated.cells;
  ...
  const handleApplyCleanup = useCallback(async () => {
    ...
    try {
      setCells(await cleanupSpriteCells(cells));
  ...
  }, [cells, cleanupSpriteCells, noBackground]);
                ...
                >
                  {imageConnections.map((c) => (
                    <option key={c.id} value={c.id}>
                      {c.name}
                      {c.model ? ` — ${c.model}` : ""}
                      {isDefaultImageGenerationConnection(c) ? " (Default)" : ""}
                    </option>
                  ))}
                </select>
              )}
