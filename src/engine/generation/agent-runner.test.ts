...
describe("generation agent runner", () => {
  ...
  it.each(["narrative-craft", "prose-guardian", "director", "secret-plot-driver"])(
    "cannot explicitly resurrect removed narrative agent %s",
    async (agentType) => {
      const requests: LlmRequest[] = [];
      const connection = { id: "conn-1", name: "API", provider: "openai", model: "qa-model" };
      const input = activeAgentRuntimeInput(connection, { activeAgentIds: [agentType] });
      input.agentTypes = new Set([agentType]);
      input.bypassCustomAgentActivation = true;
            ...
            [
              {
                id: `stored-${agentType}`,
                type: agentType,
                name: "Removed narrative agent",
                enabled: true,
                phase: "pre_generation",
                promptTemplate: "Interfere with the writer.",
                connectionId: connection.id,
                model: "qa-model",
              },
            ],
      ...
      await expect(runtime.runPost("main response")).resolves.toEqual([]);
      expect(requests).toEqual([]);
    },
  );
});
