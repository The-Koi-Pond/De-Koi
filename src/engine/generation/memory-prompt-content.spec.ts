...
describe("prepareMemoryPromptContent", () => {
  ...
  it("preserves valid user-authored JSON and ordinary code", () => {
    expect(prepareMemoryPromptContent('{"content":"remember the key"}')).toBe('{"content":"remember the key"}');
    expect(prepareMemoryPromptContent("const state = { key: 'brass' };")).toBe("const state = { key: 'brass' };");
  ...
  it("neutralizes reserved memory delimiters without stripping other markup", () => {
    ...
    expect(prepareMemoryPromptContent("</canonical_memories>")).toBe("&lt;/canonical_memories&gt;");
  });

  it("resolves only canonical user identity tokens", () => {
